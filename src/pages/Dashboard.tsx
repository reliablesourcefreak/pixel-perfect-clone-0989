...
import { Upload, ArrowRight, Zap, Layers, Tag, TrendingUp } from "lucide-react";
...
export default function Dashboard() {
  ...
  return (
    <div className="px-8 py-10 max-w-7xl mx-auto">
      {/* Hero header */}
      <div className="mb-14">
        <span className="catalog-num">Archive Dashboard — Live</span>
        <div className="mt-4 flex items-end justify-between gap-8">
          <div>
            ...
            <p className="font-mono text-xs text-muted-foreground mt-5 tracking-wide max-w-md leading-relaxed">
              Your personal AI-powered archive. Every upload is automatically analyzed, categorized, 
              tagged, and connected. A living database that grows smarter with every piece.
            </p>
          </div>
      ...
      <div className="flex gap-3 mb-12">
        ...
        <Button variant="outline" className="font-mono text-xs tracking-wide border-border hover:border-foreground" onClick={() => navigate("/timeline")}>
          View Timeline
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-px bg-border border border-border">
        ...
        {/* Latest work — hero spotlight */}
        <div className="lg:col-span-2 bg-background">
          {latestWork ? (
            <div
              className="group cursor-pointer relative"
              onClick={() => navigate(`/gallery/${latestWork.id}`)}
            >
              <div className="relative aspect-[16/9] overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 translate-y-2 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-500">
                  <span className="font-mono text-[10px] text-primary-foreground/60 tracking-widest uppercase">Latest Addition</span>
                  <h3 className="font-serif text-2xl text-primary-foreground mt-1">{latestWork.title}</h3>
                  <span className="font-mono text-[10px] text-primary-foreground/60 tracking-wide mt-2 block">
                    {format(new Date(latestWork.created_at), "MMMM d, yyyy")}
                  </span>
                </div>
              </div>
            ...
            <div className="p-16 text-center">
              ...
              <p className="font-mono text-xs text-muted-foreground tracking-wide">
                Upload your first artwork to activate the intelligence engine.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-background flex flex-col divide-y divide-border">
          <div className="p-6 flex-1 flex flex-col justify-center">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="section-label">Archive Status</span>
            </div>
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-1">
                  <span className="font-mono text-[10px] text-muted-foreground tracking-wide">Total Works</span>
                  <span className="font-serif text-lg text-foreground">{totalCount}</span>
                </div>
                <div className="h-px bg-border" />
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  <span className="font-mono text-[10px] text-muted-foreground tracking-wide">AI Analyzed</span>
                  <span className="font-serif text-lg text-foreground">{analyzedCount}</span>
                </div>
              ...
              <div>
                <div className="flex justify-between mb-1">
                  <span className="font-mono text-[10px] text-muted-foreground tracking-wide">Categories</span>
                  <span className="font-serif text-lg text-foreground">{categories.length}</span>
                </div>
                <div className="h-px bg-border" />
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  <span className="font-mono text-[10px] text-muted-foreground tracking-wide">Unique Tags</span>
                  <span className="font-serif text-lg text-foreground">{tags.length}</span>
                </div>
                <div className="h-px bg-border" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              {artworks.slice(0, 4).map((art) => (
                <div
                  ...
                  className="flex items-center gap-3 cursor-pointer group/item hover:bg-secondary transition-colors p-1.5 -mx-1.5"
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-border border border-border border-t-0">
        ...
        <div className="bg-background p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate("/gallery")} className="font-mono text-[10px] text-muted-foreground hover:text-foreground tracking-wide transition-colors">
              View All →
            </button>
          </div>
          {categories.length === 0 ? (
            <p className="font-mono text-xs text-muted-foreground tracking-wide">Upload artworks to see category breakdown.</p>
          ) : (
        ...
        <div className="bg-background p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="section-label">Tag Universe</span>
            </div>
            <span className="font-mono text-[10px] text-muted-foreground">{tags.length} tags</span>
          </div>
          {tags.length === 0 ? (
            <p className="font-mono text-xs text-muted-foreground tracking-wide">Tags will appear once artworks are analyzed.</p>
          ) : (
              ...
              {tags.map(({ tag, count }) => {
                ...
                return (
                  <button
                    ...
                    className="font-mono px-2 py-1 border border-border hover:border-foreground hover:bg-secondary transition-colors"
                    style={{ fontSize: `${Math.max(9, Math.min(13, 9 + (count / maxTag) * 4))}px`, opacity: intensity }}
                  >
                    {tag}
                ...
                <div className="relative aspect-square overflow-hidden">
                  ...
                  <div className="forensic-overlay p-3 flex flex-col justify-end font-mono text-primary-foreground">
                    <span className="text-[10px] tracking-widest uppercase opacity-60">
                      {art.analysis_status === "complete" ? "Analyzed" : art.analysis_status}
                    </span>
